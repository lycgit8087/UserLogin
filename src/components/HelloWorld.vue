<template>
  <div class="hello">
    ...
    <div class="des_view" v-if="people_type==3||people_type==2">
      <!-- 家长 -->
      ...
      <div class="teacher_view" v-if="people_type==2">
        ...
        <div class="class_view">
          <div :class="sindex==index?'action_class':''" v-for="(item,index) in student_list " :key="index" @click="change_list(index)" >{{item.class_name}}</div>
        </div>
        ...
        <div class="student_view">
          <div :class="stundent_num==index?'action_class':''" @click="change_student_list(index)" v-for="(item,index) in student_list[sindex].class_data " :key="index">
            <img :src="item.avatar" fit="cover" />
...
export default {
  ...
  methods: {
    ...
    to_send(num) {
      ...
      let sid = 0,
        ...
        if(num!=1){
          url = `https://sc.imofang.cn/cube.aspx?c=mp&module_name=aits&lr=0&at=surestatus&user_type=${people_type}&oc_id=w_0_0_1595397188728&sid=${sid}&status=1`;

        }else{
          url = `https://sc.imofang.cn/cube.aspx?c=mp&module_name=aits&lr=0&at=surestatus&user_type=${people_type}&oc_id=w_0_0_1595397188728&status=1`;
  ...
  mounted() {
    ...
    for(let i in student_list){
      for(let j in student_list[i].class_data){
        student_list[i].class_data[j].avatar=`http://files.imofang.cn${student_list[i].class_data[j].avatar}`
      }
    }
